...
สถานะโค้ดปัจจุบัน: ผู้เปิด LIFF กรอกได้เฉพาะรหัสบุคลากร ระบบจับคู่กับแถว Staff ที่มีข้อมูลทำเนียบ 7 ช่องหลักครบและไม่ได้ถูกกำหนดเป็น `INACTIVE`, เก็บคำขอเป็น `PENDING`, กัน LINE/บุคลากรผูกซ้ำ และยื่นลาไม่ได้จนผู้ดูแลอนุมัติพร้อมเหตุผล เมื่ออนุมัติระบบตั้ง `ACTIVE/APPROVED` ให้อัตโนมัติ ข้อมูลชื่อ กลุ่มงาน ตำแหน่ง และประเภทบุคลากรไม่รับจาก client แล้ว
...
ตั้ง `logs_retention_days` ตามนโยบายองค์กร (ค่าเริ่มต้น 90 วัน, ระบบรับ 30-3650 วัน) สำหรับชีต `Logs` เท่านั้น การ cleanup ใช้ trigger รายวันแยกจากงานแจ้งเตือน จึงยังทำงานเมื่อปิดการส่งข้อความเช้าและไม่กระทบงานหลัก ส่วน `AuditLog` ต้องเก็บตามนโยบายหลักฐานการเปลี่ยนแปลง/เอกสารบุคลากร และ `SecurityEvents` ใช้อายุรายการตามกลไกความปลอดภัยของระบบ ห้ามนำสองชีตนี้ไปรวมกับการลบ `Logs` อัตโนมัติ
